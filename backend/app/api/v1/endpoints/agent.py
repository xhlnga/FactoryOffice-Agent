...
from app.schemas.agent import AgentChatRequest, AgentChatResponse, ToolCallPreview
...
def chat_with_agent(request: AgentChatRequest, db: Session = Depends(get_db)) -> AgentChatResponse:
    ...
    state = run_factory_office_agent(request.message, user_id=request.user_id, db=db)
